...
void BaseDialog::RemoveTimer(W32Timer* timer)
{
...
void BaseDialog::AddTimer(W32Timer* pTimer)
{
...
DWORD W32Timer::mIdGen = 1;

W32Timer::W32Timer(BaseDialog* parent)
    : mParent(parent)
{
...
void W32Timer::Start(DWORD intervalMs)
{
    assert(mTimerId == 0);
    mId = mIdGen;
    mIdGen++;
...
void W32Timer::OnTick(std::function<void()> onTick)
{
    mOnTick = onTick;
}
