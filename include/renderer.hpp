...
namespace OpenGL
{
    ...
    {
    public:
        Texture(u32 width, u32 height, GLint format = GL_RGB)
        {
        ...
        GLuint Id() const { return mTexture; }
    private:
        GLuint mTexture = 0;
    };
    ...
    public:
        ...
        {
            GL(glGenRenderbuffers(1, &mRenderBuffer));
            Bind();
            GL(glRenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH_COMPONENT, width, height));
            GL(glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_RENDERBUFFER, mRenderBuffer));
        }

        void Bind()
        {
            GL(glBindRenderbuffer(GL_RENDERBUFFER, mRenderBuffer));
        }
        ...
        ~RenderBuffer()
        {
            if (mRenderBuffer != 0)
            {
                GL(glDeleteRenderbuffers(1, &mRenderBuffer));
            }
        }
    private:
    ...
    {
    public:
        RenderToTexture(u32 width, u32 height)
            : mTexture(width, height), mDepthBuffer(width, height)
        {
            mTexture.SetLinearFiltering();

            GL(glFramebufferTexture(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, mTexture.Id(), 0));
    ...
    private:
        OpenGL::FrameBufferObject mFbo;
        OpenGL::Texture mTexture;
        OpenGL::RenderBuffer mDepthBuffer;
    };
}
