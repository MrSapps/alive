...
TEST(Collision, NoLines)
{
    ...
    ASSERT_FALSE(Physics::raycast_map<1>(lines, { 0, 0 }, { 10, 10 }, { 0u }, &hitPoint));
}
...
TEST(Collision, NoLinesMultiInput)
{
    Physics::raycast_collision hitPoint;
    std::vector<Oddlib::Path::CollisionItem> lines;
    ASSERT_FALSE(Physics::raycast_map<2>(lines, { 0, 0 }, { 10, 10 }, { 0u, 7u }, &hitPoint));
}
...
TEST(Collision, DISABLED_LineWithinLine)
{
    ...
    ASSERT_TRUE(Physics::raycast_map<1>(lines, { 5, 0 }, { 9, 0 }, { 0u }, &hitPoint));
...
TEST(Collision, NearestHitPointWins)
{
    ...
    ASSERT_TRUE(Physics::raycast_map<1>(lines, { 1957, 1090 }, { 1957, 1590 }, { 0u }, &hitPoint));
