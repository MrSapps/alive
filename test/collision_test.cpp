...
{
    Physics::RaycastCollision hitPoint;
...
TEST(Collision, DISABLED_LineWithinLine)
{
    ...
    ASSERT_EQ(hitPoint.mIntersection.x, 0);
    ASSERT_EQ(hitPoint.mIntersection.y, 0);
}
...
TEST(Collision, NearestHitPointWins)
{
    ...
    ASSERT_EQ(hitPoint.mIntersection.x, 1957);
    ASSERT_EQ(hitPoint.mIntersection.y, 1140);
}
